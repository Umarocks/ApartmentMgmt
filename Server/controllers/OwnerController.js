...
const OwnerController = {
    ...
    viewProperties: async (req, res) => {
        try {
            const result = await pool.query('SELECT * FROM Apartment WHERE owner_id = $1', [req.user.owner_id]);
    ...
    viewPayments: async (req, res) => {
        try {
            ...
            const result = await pool.query('SELECT * FROM Payment WHERE owner_id = $1', [req.user.owner_id]);
            res.json(result.rows);
        } catch (error) {
